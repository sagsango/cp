...
ll arr[nax], dp[nax][2][2], n;

void solve(){
	// [ ith element ][ pariy of curr sequence number ][ parity of index ] = max sum.
	cin >> n;
	for(int i=0;i<n;++i){
		cin >> arr[i];
	}
	...
	for(int i=0;i+1<n;++i){
		for(int j=0;j<2;++j){
			for(int k=0;k<2;++k){
				...
				// add in the curr sequence
				p = ( j + (k^1) ) % 2 ? -arr[i+1] : +arr[i+1];
				smax( dp[i+1][j][k^1], dp[i][j][k] + p );


				// make a new sequence
				p = ( (j^1) + 1 ) % 2 ? -arr[i+1] : +arr[i+1];
				smax( dp[i+1][j^1][1], dp[i][j][k] + p );
