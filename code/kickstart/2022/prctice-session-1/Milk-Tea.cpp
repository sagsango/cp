...
struct box{
	int happy;
	string mask;
	box(int happy,string mask): happy(happy), mask(mask) {}
...
int main(){
    ...
    for(int tt=1;tt<=T;++tt){
			...
			int dp[p][2]; memset(dp, 0, sizeof(dp));
			for(int i=0;i<n;++i){
				cin >> arr[i];
				for(int j=0;j<p;++j){
					dp[j][ arr[i][j] - '0' ] += 1;
			...
			pq.push({happy,mask});
			vis.insert(mask);
			while( st.count(pq.top().mask) ){
				string mask = pq.top().mask;
				int happy = pq.top().happy;
				pq.pop();
				for(int i=0;i<p;++i){
					char c = dp[i][0] > dp[i][1] ? '0' : '1';
					int decr = dp[i][0] > dp[i][1] ? dp[i][1] - dp[i][0] : dp[i][0] - dp[i][1];
					if( c == mask[i] ){
						string tmask = mask;
						tmask[i] = c == '0' ? '1' : '0';
						if( !vis.count(tmask) ){
							pq.push({happy+decr,tmask});
							vis.insert(tmask);
						}
					}
				}
