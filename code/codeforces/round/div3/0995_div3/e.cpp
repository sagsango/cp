...
int main(){
    ...
    while (t--) {
        ...
        ll asum = 0, bsum = 0, max_profit = 0;
        int m = n << 1;
        for (int i=0; i<m;) {
            int j = i;
            while (j < m && arr[i].first == arr[j].first) {
                int x = arr[j].first;
                int is_b = arr[j].second;
                if (is_b) {
                    bsum += 1;
                } else {
                    bsum -= 1;
                ...
                //bug6(j, x, is_b, asum, bsum, x*( asum + bsum));
                ++j;
            }
            ...
            if (neg_review <= k) {
                max_profit = max(max_profit, 1ll * arr[i].first*(asum + bsum));
            }
