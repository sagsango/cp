...
int main(){
	...
	while (t--) {
		...
		tot_area = 0;
		for (int i=0; i<n; ++i) {
			int next_y = i + 1 < n ? arr[i+1] : 2e9 + 7;
			ld diff = (arr[i] + h) - next_y;
			bug3(next_y, diff, arr[i])
			tot_area += area(d, h);
			if (diff > 0) {
				tot_area -= area(diff, diff) * (d/h);
			}
		}
		cout << setprecision(10) << tot_area/2 << endl;
	}
}
