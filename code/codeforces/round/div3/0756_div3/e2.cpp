...
int main(){
	...
	while( t-- ){
		...
		std::function<vector<int>(vector<int>)> bfs = [&](vector<int>source){
			vector<int>dis(n+1,1e9);
			priority_queue< ii, vector<ii>, greater<ii> >q;
			// multiple guard case: 
			// if u-v can gaurd same vertex.
			// then all the vertex gaurded by u, can be guarded be v
			// and vice-versa
			// so take only one of them
			// so priority_queue< <dis,vertex> > will take care
			// we will always take min(u,v), as a guard
			for(auto s:source){
				dis[s] = 0;
				guard[s] = s;
				q.push({dis[s],s});
		...
		int ok = 0;
		queue<int>q;
		vector<int>vis(n+1),dis(n+1,1e9);
		q.push(1);
		vis[1] = 1;
		dis[1] = 0;
		set<int>Guard;
