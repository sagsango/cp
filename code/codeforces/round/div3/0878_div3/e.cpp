...
string s[2], ans[2] = {"NO", "YES"};
int t, n, block_time, nq, e_cnt, in[5];
...
int main(){
	...
	while (t--) {
		...
		cin >> block_time >> nq;
		e_cnt = 0, n = s[0].length();
		for (int i=0; i<n; ++i) {
			add(i);
		}
		for (int timer=0; timer<nq; ++timer) {
			cin >> in[0];
			switch (in[0]) {
				case 1:
					cin >> in[2];
				...
				case 2:
					...
					remove(in[2]);
					remove(in[4]);
					swap(s[in[1]][in[2]], s[in[3]][in[4]]);
					add(in[2]);
					add(in[4]);
					break;
				case 3:
					while (!q.empty() && q.front().second + block_time <= timer) {
						add(q.front().first);
						q.pop();
					}
					bug3(q.size(), e_cnt, n)
					cout << ans[(q.size() + e_cnt == n)] << endl;
			}
		}
		while (!q.empty()) {
			q.pop();
