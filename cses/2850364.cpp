...
int main(){
	...
	cin >> n >> k;
	vector<ld>dp(k+1);
	for(int max_candy = 1; max_candy <= k; ++max_candy){
		ld less_than_or_equal_to_max_candy = 1;
		ld less_than_max_candy = 1;
		for(int boys = 1; boys <= n; ++boys){
			less_than_or_equal_to_max_candy *= ( 1.00l * max_candy )/ k;
			less_than_max_candy *= ( 1.00l * ( max_candy -1 ))/ k;
		}
		dp[max_candy] = less_than_or_equal_to_max_candy - less_than_max_candy;
	}
	ld expected_money = 0;
	for(int i=1;i<=k;++i){
		expected_money += i * dp[i];
	}
	cout<<fixed<<setprecision(6)<<expected_money;
