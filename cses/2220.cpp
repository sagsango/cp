...
/*
	 dp[len][last_digit][is_non_zero][is_equal or less] = ways;
*/

ll dp[25][10][2][2], num_i ;
vector<int> num_d, ways(2);

vector<int> num_digits(ll num){
...
solve(ll num) {
	ll ways = 0;
	vector<int>num_d = num_digits(num);
	...
	for	(int i=0; i<20; ++i) {
		for (int j=0; j<10; ++j) {
			for (int k=0; k<2; ++k) {
				...
				for (d = 0; d<num_d[i]; ++d) {
					if (!k || j != d)
						dp[i+1][d][k||d][0] += dp[i][j][k][1];
				}
				if (!k || j !=d) {
					dp[i+1][d][k||d][1] += dp[i][j][k][1];
				}
				for (d=0; d<10; ++d) {
					if(!k || j != d) {
						dp[i+1][d][k||d][0] += dp[i][j][k][0];
					}
				}
			}
		}
...
int32_t main(){
	...
	ll l, r; cin >> l >> r;
	cout << solve(r) - (l ? solve(l-1) : 0) << endl;
}
