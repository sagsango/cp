...
int main(){
	...
	for(int i=0;i<n;++i){
		...
		order.push_back(sum), order.push_back(sum+300);
	}
	...
	for(int i=0;i<n;++i){
		...
		int x = lower_bound(order.begin(),order.end(),sum) - order.begin() + 1;
		update(x,+1);
	}	
	for(int i=0;i<n;++i){
		int sum = 0;
		...
		int x = lower_bound(order.begin(),order.end(),sum+000) - order.begin() + 1;
		int y = lower_bound(order.begin(),order.end(),sum+300) - order.begin() + 1;
		update(x,-1);
		int rank = ( n - 1 ) - ( query(y) ) + 1;
		update(x,+1);
