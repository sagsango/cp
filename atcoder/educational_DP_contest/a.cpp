...
int32_t main(){
	...
	// dp[length][remender][is_equal, or less] = ways
	dp[0][0][1] = 1;
	for(int i=0; i<n; ++i) {
		for (int j=0; j<m; ++j) {
			int d;
			for (d=0; d <(s[i]-'0'); ++d) {
				(dp[i+1][(d+j)%m][0] += dp[i][j][1]) %= mod;
			}
			(dp[i+1][(d+j)%m][1] += dp[i][j][1]) %= mod;
			for(d=0; d<10; ++d) {
				(dp[i+1][(d+j)%m][0] += dp[i][j][0]) %= mod;
			}
		}
	}

	cout << (dp[n][0][0] + dp[n][0][1] - 1 /* zero */ + mod) % mod << endl;
}
