...
#define bugnm( x , n , m )          do{ cerr << (#x) << endl; for(int i=0;i<n;i++){ for(int j=0;j<m;j++) cerr << x[i][j] << "    "; } cout << endl; }WHILE_ONCE;
#else
#define bug1( x )
#define bug2( x , y )
#define bug3( x , y , z )
...
// dp[ index ][ length ] = no of diff subsequence of length = length
const int nax = 105;
int dp[nax][nax], seen[26], n, k;
string s;

// because we are getting considering only 100.
// we can make last 100, subsequences, that will resolve this problem,
// so use bfs from back, [ see editorial ]
int main(){
	...
	dp[0][0] = 1;
	for(int i=0;i<n;++i){
		//privious subsequence
		for(int j=0;j<n;++j){
			dp[i+1][j] += dp[i][j];
		}
		//privious subsequence + s[i]
		for(int j=0;j<n;++j){
			dp[i+1][j+1] += dp[i][j];
