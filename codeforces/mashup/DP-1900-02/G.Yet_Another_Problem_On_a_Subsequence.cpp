...
int main(){
	...
	for(int i=0;i<n;++i){
		cin >> a[i];
		bug3(i,dp[i][0],dp[i][1]);
		//dp[i+1][0] = add(dp[i+1][0],dp[i][0]);
		for(int j=0;j<n;++j){
			dp[i+1][j] = add(dp[i+1][j],dp[i][j+1]);
			dp[i+1][j] = add(dp[i+1][j],dp[i][j+0]);
		}
		if( a[i] <= n && a[i] > 0 ){
			dp[i+1][a[i]] = add(dp[i+1][a[i]],dp[i][0]);
		}
	}
	cout << sub(dp[n][0],1) << endl;
}
