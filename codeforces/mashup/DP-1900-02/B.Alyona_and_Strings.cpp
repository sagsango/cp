...
int dp[1002][1002][12][2], n, m, K;
//dp[length from a][length from b][substring cnt][ongoing substring or not] = max length
string a, b;
int main(){
	...
	for(int i=0;i<=n;++i){
		for(int j=0;j<=m;++j){
			for(int k=0;k<=K;++k){
				bug5(i,j,k,dp[i][j][k][0],dp[i][j][k][1]);
				smax(dp[i+1][j][k][0],dp[i][j][k][0]);
				smax(dp[i][j+1][k][0],dp[i][j][k][0]);
				if( a[i] == b[j] ){
					smax(dp[i+1][j+1][k+1][1],dp[i][j][k][0]+1);
					smax(dp[i+1][j+1][k+1][0],dp[i][j][k][0]+1);
					smax(dp[i+1][j+1][k+0][1],dp[i][j][k][1]+1);
					smax(dp[i+1][j+1][k+0][0],dp[i][j][k][1]+1);
				}
			}
		}
	}
