...
int main(){
	...
	while (test--) {
		...
		set <int> horizontal[n+2], vertical[m+2];
		for (int i=0; i<r; ++i) {
			cin >> t >> d >> coord;
			switch (d) {
				case 1:
			...
			for (int j=0; j<=m; ++j) {
				for (int k=0; k<=r; ++k) {
					if (dp[i][j][k]) {
						t = i + j + k + 1;
						/* i -> i + 1 */
						if (horizontal[i+1].find(t) == horizontal[i+1].end() &&
								vertical[j].find(t) == vertical[j].end()) {
							bug5(t, i, j, k, i+1);
							dp[i+1][j][k] = 1;
						}
						/* j -> j + 1 */
						if (vertical[j+1].find(t) == vertical[j+1].end() &&
								horizontal[i].find(t) == horizontal[i].end()) {
							bug5(t, i, j, k, j+1);
							dp[i][j+1][k] = 1;
						}
						/* stay */
						if (horizontal[i].find(t) == horizontal[i].end() &&
								vertical[j].find(t) == vertical[j].end()) {
							bug5(t, i, j, k, k+1);
							dp[i][j][k+1] = 1;
						}
