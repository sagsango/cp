...
int main(){
	...
	while( t-- ){
		int a, b, k; cin >> a >> b >> k;
		int g = gcd(a,b);
		int mn_step[2], mx_step[2];
		mn_step[0] = g != a;
		mn_step[1] = g != b;

		mx_step[0] = find_mx_step(a);
		mx_step[1] = find_mx_step(b);
		//bug6(a,b,mn_step[0],mn_step[1],mx_step[0],mx_step[1]);
		if(  k < mn_step[0] + mn_step[1] || k > mx_step[0] + mx_step[1] || ( k == 1 && a == b)){
			cout << "NO" << endl;
			continue;
		}
		cout << "YES" << endl;
