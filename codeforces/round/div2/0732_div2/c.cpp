...
// see editorial : very good explanation

void count(vector<int>&arr,map<pair<int,int>,int>&mp){
...
int main(){
	...
	while( t-- ){
		int n; cin >> n;
		map<pair<int,int>,int>before, after;
		vector<int>arr(n);
		for(int i=0;i<n;++i){
			cin >> arr[i];
			before[{arr[i],0}]= before[{arr[i],1}] = after[{arr[i],0}] = after[{arr[i],1}]= 0;
		}
		count(arr,before);
		sort(arr.begin(),arr.end());
		count(arr,after);
		cout << (  before == after ? "YES" : "NO" ) << endl;
	}
}
