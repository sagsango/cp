...
const int nax = 1e5+5;
int par[nax][20], dar[nax][20], arr[nax], parx[20], pary[20], darx[20], dary[20], n, q;
int main(){
	...
	for(int i=1;i<20;++i){
		for(int u=1;u<=n;++u){
			...
			dar[u][i] = dar[dar[u][i-1]][i-1];
		}
	}
	for(int u=1;u<=n;++u){
		bug3(u,par[u][0],dar[u][0]);
	}
	...
	while( q-- ){
		...
		if( t == 1 ){
			...
			for(int i=0;i<20;++i){
				//swap(par[darx[i]][i],par[dary[i]][i]);
				//swap(dar[parx[i]][i],dar[pary[i]][i]);
			}
			for(int u=1;u<=n;++u){
				bug3(u,par[u][0],dar[u][0]);
			}
			...
			for(int i=0;i<20;++i){
				...
				dar[x][i] = dary[i];
				dar[y][i] = darx[i];
			//	swap(par[x][i],par[y][i]);
			//	swap(dar[x][i],dar[y][i]);
