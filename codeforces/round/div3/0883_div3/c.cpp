...
int t, n, m, h, penalty, score, t_spend;
signed main(){
	...
	while (t--) {
		...
		for (int i=0; i<n; ++i) {
			...
			rank.push_back({score, penalty, i});
		}
		/*
		...
		*/
		int better_player = 0;
		for (int i=1; i<n; ++i) {
			bug5(rank[0].score, rank[0].penalty, rank[i].score, rank[i].penalty, (rank[0] < rank[i]));
			if (rank[i] < rank[0]) {
				better_player++;
			}
		}
		cout << better_player + 1 << endl;
	}
}
