...
#define	LESS	0
			...
			for (int bit = 30; bit >= 0; --bit) {
				if (n >> bit & 1) {
					dp[bit][EQUAL] = 1 * dp[bit+1][EQUAL];
					dp[bit][LESS]  = 1 * dp[bit+1][EQUAL] + 2 * dp[bit+1][LESS];
				}
				else {
					dp[bit][EQUAL] = 1 * dp[bit+1][EQUAL];
					dp[bit][LESS] = 2 * dp[bit+1][LESS];
				}
			}
			cout << (dp[0][LESS] + dp[0][EQUAL]) << endl;
		}
	}
}
