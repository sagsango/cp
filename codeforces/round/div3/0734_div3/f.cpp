...
int nck(vector<int>arr,int k){
	...
	for(int i=0;i<=n;++i){
		for(int j=0;j<=k;++j){
			if(i<n){
				dp[i+1][j] = add(dp[i+1][j], mul(dp[i][j],1));
			}
			if(i<n && j<k){
				dp[i+1][j+1] = add(dp[i+1][j+1], mul(dp[i][j],arr[i]));
			}
		}
	}
	return dp[n][k];
}
void dfs(int u,int p,int dep,vector<int>&cnt){
	cnt[dep]+=1;
	for(auto v:g[u]){
		if( v!= p ){
			dfs(v,u,dep+1,cnt);
	...
	while( t-- ){
		...
		for(int i=0;i<n;++i){
			...
			for(auto u:g[i]){
				...
				dfs(u,i,0,tmp);
				for(int i=0;i<n;++i){
					arr[i].push_back(tmp[i]);
				}
			}
			for(int j=0;j<n;++j){
				ans = add(ans, nck(arr[j],k));
			}
		}
		cout << ans << endl;
	}
